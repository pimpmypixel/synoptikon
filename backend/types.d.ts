...
declare module 'motia' {
  ...
  interface Handlers {
    ...
    'CreatePoster': ApiRouteHandler<{ type: 'map' | 'your-sky'; city?: string; country?: string; lat?: number; lon?: number; googleMapsUrl?: string; theme: string; distance: number; border?: number; format: 'png' | 'svg' | 'pdf'; landscape: boolean; titleFont?: string; subtitleFont?: string; paperSize?: string; rotation?: number; widthCm?: number; heightCm?: number; timestamp?: string; observationPoint: 'current' | 'specified'; celestialObjects?: { stars: boolean; planets: boolean; moon: boolean; constellations: boolean; zodiac: boolean; grid: boolean; deepSkyObjects: boolean }; projection?: { type: 'stereographic' | 'polar'; centerLat?: number; centerLon?: number; fov: number; northUp: boolean }; styling?: { starColors: 'realistic' | 'temperature' | 'monochrome'; starMagnitudes?: { minMagnitude: number; maxMagnitude: number }; constellationLines: boolean; constellationLabels: boolean; gridLines: boolean } }, ApiResponse<200, { success: boolean; jobId: string; message: string }> | ApiResponse<400, { error: string }>, { topic: 'create-poster'; data: never }>
    'CleanPosterJobs': ApiRouteHandler<{ mode: 'stale' | 'errors' | 'all' }, ApiResponse<200, { removed: number; message: string }>, never>
  }
